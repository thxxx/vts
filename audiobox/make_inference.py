...
if __name__ == "__main__":
    ...
    for i in range(20000, len(df)):
        ...
        for idx, audio in enumerate(oa):
            fp = re.sub("/workspace/data/ss/", "", data['audio_path'])
            fp = '.'.join(fp.split(".")[:-1])
            fp = '_'.join(fp.split("/"))
            torchaudio.save(f'./outputs/{fp}_{idx}.wav', audio, sample_rate=44100)
            
            claps.append({
                'audio_path': f'./outputs/{fp}_{idx}.wav',
                'cosiine_similarity': 0,
                'new_caption': caption,
                'caption': data['caption'],
                'duration': data['duration']
            })

        # text_embed = clmodel.get_text_embedding([caption])
        
        # all_embeds = [
        #     torchaudio.load(f'./outputs/{fp}_0.wav'[0][0]),
        #     torchaudio.load(f'./outputs/{fp}_1.wav'[0][0])
        # ]
        
        # for idx, audio_embed in tqdm(enumerate(all_embeds)):
        #     audio_embed = model.get_audio_embedding_from_data(x = [np.array(audio_embed)])
        #     simil = cosine_similarity(audio_embed, text_embed[0])
        ...
        #     })
        if i%500 == 499:
            pd.DataFrame(claps).to_csv("running.csv")
