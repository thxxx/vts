...
for video_file in tqdm(video_files):
    ...
    # 영상 전처리
    video_frames = rearrange(video_frames, 't h w c -> 1 t c h w').tile(2, 1, 1, 1, 1)  # (1, C, T, H, W)
    video_frames = video_frames.to(torch.float32).to('cuda') / 255.0

    try:
        with torch.no_grad():
            out = encode_video_with_sync(video_frames)
...
for fp in tqdm(files):
    ...
    try:
        ...
        np.save(os.path.join(ltx_latents_dir, fp[:-4] + ".npy"), latent[0])
    except Exception as e:
        print(f"\n\n ERROR : {e} \n\n")