...
class CLAP(nn.Module):
    def __init__(
        ...
        # self.sig_scale = nn.Parameter(torch.tensor(10.0, dtype=torch.float32))
        # self.sig_bias = nn.Parameter(torch.tensor(-10.0, dtype=torch.float32))
    ...
    def encode_text(self, text, device):
        # print("text_branch_type : ", self.text_branch_type)
        if self.text_branch_type == "transformer":
        ...
        elif self.text_branch_type == "roberta": # 지금 사용하는건 이 roberta
            x = self.text_branch(
                input_ids=text["input_ids"].to(device=device, non_blocking=True),
                attention_mask=text["attention_mask"].to(
                    device=device, non_blocking=True
    ...
    def forward(self, audio, text, device=None):
        ...
        middle_check = self.encode_audio(audio, device=device)["embedding"]
        audio_features = self.audio_projection(middle_check) # 오디오 인코딩
        audio_features = F.normalize(audio_features, dim=-1)

        text_features = self.encode_text(  # 텍스트 인코딩
            text, device=device
        )
    ...
    def get_audio_embedding(self, data):
        ...
        audio_embeds = self.encode_audio(input_dict, device=device)["embedding"] # 여기만 바뀌면 되지. 
