...
class AudioBoxModule(LightningModule):
    ...
    def forward(
        ...
        try:
            ...
            times = sample_lognorm(batch, self.device, audio_enc.dtype)
            # torch.rand((batch,), dtype=audio_enc.dtype, device=self.device) # torch.rand는 0~1 uniform sampling
            t = rearrange(times, "b -> b () ()")
            w = (1 - (1 - self.sigma) * t) * audio_x0 + t * audio_enc

            cond_drop_mask = prob_mask_like((batch, 1), 0.5, self.device)
            ...
            # random condition drop(zero) for cfg
    ...
    def training_step(
        ...
        return self.single_step(batch[:-1], "train")
