...
from vocos.encodec.modules.conv import SConv1d, SConvTranspose1d
from vocos.encodec.modules.lstm import SLSTM
...
class SEANetResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        activation: str = "ELU",
        activation_params: dict[str, float] = {"alpha": 1.0},
        ...
        act: type[nn.Module] = getattr(nn, activation)
        hidden = dim // compress
        block = []
...
class SEANetEncoder(nn.Module):
    ...
    def __init__(
        ...
        for ratio in self.ratios:
            ...
            modules.append(act(**activation_params))
            modules.append(
                SConv1d(
                    mult * n_filters,
                    mult * n_filters * 2,
        ...
        if lstm:
            modules.append(SLSTM(mult * n_filters, num_layers=lstm))
...
class SEANetDecoder(nn.Module):
    ...
    def __init__(
        ...
        if final_activation is not None:
            final_act: type[nn.Module] = getattr(nn, final_activation)
            final_activation_params = final_activation_params or {}
            modules.append(final_act(**final_activation_params))
        self.model = nn.Sequential(*modules)
