...
class OobleckResidualUnit(nn.Module):
    ...
    def forward(self, hidden_state):
        ...
        Args:
            hidden_state (`Tensor` of shape `(batch_size, channels, time_steps)`):
                Input tensor .

        Returns:
            output_tensor (`Tensor` of shape `(batch_size, channels, time_steps)`)
...
class OobleckDiagonalGaussianDistribution(object):
    ...
    def sample(self) -> Tensor:
        ...
        sample = torch.randn_like(self.std)
        ...
        for stride_index, stride in enumerate(strides):
            self.block += [
                OobleckEncoderBlock(
                    ...
                    * channel_multiples[stride_index + 1],
                    stride=stride,
                )
            ]
...
class AutoencoderOobleck(nn.Module):
    ...
    def encode(self, x: Tensor) -> OobleckDiagonalGaussianDistribution:
        """
